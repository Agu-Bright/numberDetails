...
const detail = require("./server/model/number");
...
app.post("/upload", async (req, res) => {
  try {
    ...
    fileName.mv(`${__dirname}/server/files/${fileName.name}`, function (err) {
      if (err) {
        return res.status(500).send(err);
      }
      console.log("successfully uploaded the file");
    });
    fs.createReadStream(`./server/files/${fileName.name}`)
...
app.get("/getdetails", async (req, res) => {
  try {
    ...
    fsExtra.emptyDirSync("./server/files");
